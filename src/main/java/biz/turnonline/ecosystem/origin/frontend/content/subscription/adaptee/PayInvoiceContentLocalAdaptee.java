...
/**
 * {@link PayInvoiceContent} local adaptee implementation.
 *
 * @author <a href="mailto:medvegy@turnonline.biz">Aurel Medvegy</a>
 */
@Singleton
public class PayInvoiceContentLocalAdaptee
        extends GetExecutorCachedContentAdaptee<PayInvoiceContent>
{
    private final ObjectMapper mapper;

    @Inject
    public PayInvoiceContentLocalAdaptee( @TurnOnlineClient Cache cache,
    ...
    @Override
    public PayInvoiceContent executeGet( @Nonnull Object request,
            ...
            throws IOException
    {
        ...
        RawPayInvoiceContent raw = ofy().load().type( RawPayInvoiceContent.class ).id( name ).now();
        if ( raw == null )
        {
    ...
    protected String composeKey( @Nonnull Identifier identifier,
                                 ...
                                 @Nullable Locale locale )
    {
        return naming().composeFullName( PayInvoiceContent.class, identifier, locale );
    }

    @Subscribe
    public void updateCache( PayInvoiceContentUpdateEvent event )
    {
